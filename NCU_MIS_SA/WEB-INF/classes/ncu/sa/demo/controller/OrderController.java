...
public class OrderController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        /** 建立一個新的訂單物件 */
        Order od = new Order(member_name, payment_type, email, address, phone);

        /** 將每一筆訂單細項取得出來 */
        for(int i=0 ; i < item.length() ; i++) {
            String proposaloption_id = item.getString(i);

            /** 透過 ProposalOptionHelper 物件之 getById()，取得產品的資料並加進訂單物件裡 */
            ProposalOption po = poh.getProposalOptionById(proposaloption_id);
            od.addOrderProposalOption(po);
        }
        ...
        od.setOrderProposalOptionId(result.getJSONArray("order_proposaloption_id"));
